...
/**
 * Utility class providing octal string related utility functions.
 */
public final class OctalStrings {

	private OctalStrings() {
	...
	private static class IntValueParser extends ValueParser {
		...
		protected void shift(int value) {
			if ((this.parsedValue & 0xe0000000) != 0) {
				throw new FunctionException(
						new IOException("Octal string exceeds integer limit: \"" + Strings.encode(this.s) + "\""));
			}
			this.parsedValue = (this.parsedValue << 3 | value);
		}
	...
	private static class LongValueParser extends ValueParser {
		...
		@Override
		protected void shift(int value) {
			if ((this.parsedValue & 0xe000000000000000l) != 0) {
				throw new FunctionException(
