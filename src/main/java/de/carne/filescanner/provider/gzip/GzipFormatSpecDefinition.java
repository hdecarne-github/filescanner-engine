...
final class GzipFormatSpecDefinition extends FormatSpecDefinition {
	...
	private Lazy<CompositeSpec> gzipHeaderSpec = resolveLazy("GZIP_MEMBER", CompositeSpec.class);

	private Lazy<ByteSpec> gzipMemberFlg = resolveLazy("MEMBER_FLG", ByteSpec.class);
	...
	private String decodedInputName() {
		int flg = ByteHelper.toUnsignedInt(this.gzipMemberFlg.get().get());

		return ((flg & 0x08) == 0x08 ? "???" : FileUtil.splitPath(AttributeSpecs.INPUT_NAME.get())[1]);
	}
